...
// PRINT_FUNC_NAME_();
  ...
  void forward_ernie(const DataType_ *from_tensor,
                  const DataType_ *memory_tensor,
                  DataType_ *key_cache_,
                  DataType_ *value_cache_,
                  DataType_ *key_mem_cache_,
      ...
      if (normalization_before_) {
        ...
        if (is_cross_attention == true) {
          ...
          */
          ...
          check_cuda_error(cudaGetLastError());
#endif
          layer_norm(norm_masked_output_buf_,
                   param_.pre_ffn_layernorm.gamma,
                   param_.pre_ffn_layernorm.beta,
                   norm_from_tensor_buf_,
                   m,
                   hidden_units_,
                   param_.stream);
          ...
          add_bias_input_layernorm_2_kernelLauncher(
              norm_masked_output_buf_,
              param_.post_ffn_layernorm.gamma,
              param_.post_ffn_layernorm.beta,
              param_.self_attention.attention_output_weight.bias,
              ffn_out_buf_, // need a buf here
              decoder_output,
              m,
              hidden_units_,
              param_.stream);
