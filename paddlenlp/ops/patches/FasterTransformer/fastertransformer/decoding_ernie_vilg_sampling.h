...
class ErnieVilgDecodingSampling {
private:
...
public:
  ErnieVilgDecodingSampling(const IAllocator &allocator,
    ...
    K_cache_[0] = K_mem_cache_[args_.decoder_layers_ - 1] + mem_cache_size +
  ...
  void forward(const DecoderInitParam<DataType_> *param,
    ...
    */
    ...
    for (uint step = 1; step <= args_.seq_len_; ++step) {
      if (args_.normalization_before_) {
        if (args_.prefix_lm_) {
          ernie_vilg_embeddings_kernel_launcher(from_tensor_[0],
                                     decoding_params.embedding_table,
                                     decoding_params.row_position_encoding_table,
                                     decoding_params.col_position_encoding_table,
                                     word_ids_buf_, // ->img_ids_buf_f
                                     step,
                                     args_.batch_size_,
                                     args_.hidden_units_,
                                     args_.pos_bias_,
                                     decoding_params.stream,
                                     decoding_params.img_size);
        } else {
          if (args_.is_mbart_) {
  ...
  virtual ~ErnieVilgDecodingSampling() {
    delete[] K_cache_;
    delete[] V_cache_;
    delete[] K_mem_cache_;
    delete[] V_mem_cache_;
