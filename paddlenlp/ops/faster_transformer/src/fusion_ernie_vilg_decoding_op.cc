...
#include "fusion_ernie_vilg_decoding_op.h"
#include "pd_traits.h"


std::vector<paddle::Tensor> ErnieVilgDecodingForward(
    const paddle::Tensor& src_ids,
    const paddle::Tensor& pos_ids,
    const paddle::Tensor& img_ids,
    const paddle::Tensor& img_pos_ids_row,
    const paddle::Tensor& img_pos_ids_col,
    const std::vector<paddle::Tensor>& input_mask,  // mask_row, mask_col, mask_conv
    const std::vector<paddle::Tensor>& cache_idxes,
    const paddle::Tensor& is_encoding,
    const paddle::Tensor& is_decoding,
    const paddle::Tensor& word_embedding,   // weight beginning
    const paddle::Tensor& pos_embedding_weight,
    const paddle::Tensor& img_emb_weight,
    const paddle::Tensor& img_pos_embedding_row,
    const paddle::Tensor& img_pos_embedding_col,
    const std::vector<paddle::Tensor>& pre_encoder_ln_weight,  // ln in pre_encoder weight
    const std::vector<paddle::Tensor>& pre_encoder_ln_bias,    // ln in pre_encoder bias
    const std::vector<paddle::Tensor>& pre_att_ln_weight, // ln in pre_encoder weight
    const std::vector<paddle::Tensor>& pre_att_ln_bias, // ln in pre_encoder bias
    ...
    const std::vector<paddle::Tensor>& self_out_weight, // multi_head_att_output_fc
    const std::vector<paddle::Tensor>& self_out_bias,   // multi_head_att_output_fc
    const std::vector<paddle::Tensor>& post_att_ln_weight, // post att ln weight
    const std::vector<paddle::Tensor>& post_att_ln_bias,   // post att ln bias
    const std::vector<paddle::Tensor>& pre_ffn_ln_weight,   // pre ffn ln weight 
    const std::vector<paddle::Tensor>& pre_ffn_ln_bias,     // pre ffn ln bias
    const std::vector<paddle::Tensor>& ffn_ln_weight,   // ffn ln 
    const std::vector<paddle::Tensor>& ffn_ln_bias,     // ffn ln
    const std::vector<paddle::Tensor>& ffn_inter_weight, // ffn fc_0
    const std::vector<paddle::Tensor>& ffn_inter_bias,   // ffn fc_0
    const std::vector<paddle::Tensor>& ffn_out_weight,   // ffn fc_1
    const std::vector<paddle::Tensor>& ffn_out_bias,     // ffn fc_1
    const std::vector<paddle::Tensor>& post_ffn_ln_weight,   // post ffn ln weight
    const std::vector<paddle::Tensor>& post_ffn_ln_bias,     // post ffn ln bias
    const std::vector<paddle::Tensor>& post_encoder_ln_weight,   // post encoder ln weight
    const std::vector<paddle::Tensor>& post_encoder_ln_bias,     // post encoder ln bias
    const paddle::Tensor& mask_img_lm_trans_fc_weight,
    const paddle::Tensor& mask_img_lm_trans_fc_bias,
    const paddle::Tensor& mask_img_lm_trans_layer_norm_weight,
    const paddle::Tensor& mask_img_lm_trans_layer_norm_bias,
    const paddle::Tensor& mask_img_lm_out_fc_weight,
    const paddle::Tensor& mask_img_lm_out_fc_bias,  // weight ending
  ...
  if (input_ids.place() == paddle::PlaceType::kGPU) {
    ...
    return ErnieVilgDecodingCUDAForward(input_ids,
                                      attn_mask,
                                      mem_seq_length,
                                      type_id,
                                      decoder_type_id,
...
std::vector<std::vector<int64_t>> ErnieVilgDecodingInferShape(
...
std::vector<paddle::DataType> ErnieVilgDecodingInferDtype(
...
PD_BUILD_OP(fusion_ernie_vilg_decoding)
    .Inputs({"InputIds",
             "AttnMask",
             "MemSeqLen",
             "TypeIds",
    ...
    .Attrs({"decoding_strategy: std::string",
            ...
            "early_stopping: bool",
            "min_length: int"})
    .SetKernelFn(PD_KERNEL(ErnieVilgDecodingForward))
    .SetInferShapeFn(PD_INFER_SHAPE(ErnieVilgDecodingInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(ErnieVilgDecodingInferDtype));
